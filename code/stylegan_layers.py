...
class NoiseLayer(nn.Module):
    ...
    def forward(self, x, noise=None):
        ...
        if noise is not None:
            if noise.size(-1) != x.size(-1) or noise.size(-2) != x.size(-2):
                noise = F.interpolate(noise, size=(x.size(2), x.size(3)), mode='bilinear', align_corners=False)
...
class G_synthesis(nn.Module):
    ...
    def forward(self,dlatents_in, return_activations=False):
        activations = []
